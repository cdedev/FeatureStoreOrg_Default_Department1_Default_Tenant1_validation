	...
	import (
		...
		"strconv"
		"strings"
		parser "FeatureStoreOrg_Default_Department1_Default_Tenant1_validation/Go/d6e7f7a7-351f-4388-b18a-27d41a494660/1669656174771/Sleeping_hours/parser"
	...
	type ValidationListener struct {
		*parser.BaseSleeping_hoursListener
	}
	...
	func getGrammarFile() string{
		...
		file := `grammar Sleeping_hours;

expression :  sleeping_hours EOF ;

sleeping_hours : SLEEPING_HOURS ;

SLEEPING_HOURS :SPLITKEY  OWNKEY [0-9.]* ;
...
`
	...
	func ValidateInput(input string,runID string,lineHashMap map[int]string) (bool,ErrorInfo){
		...
		lexer:=parser.NewSleeping_hoursLexer(in)
		if outerMap, ok := ErrorHandleMap.Load(runID); ok {
			if value, canConvert := outerMap.(bool); canConvert {
		...
		parser:=parser.NewSleeping_hoursParser(stream)
		if outerMap, ok := ErrorHandleMap.Load(runID); ok {
			if value, canConvert := outerMap.(bool); canConvert {
	...
	func HandleCustomError(lineHashMap map[int]string,index int,value string,errorMessage string)ErrorInfo{
		...
		errorInfo.RuleName = "Sleeping_hours"

	...
	import (
		...
		"strconv"
		"strings"
		parser "FeatureStoreOrg_Default_Department1_Default_Tenant1_validation/Go/d6e7f7a7-351f-4388-b18a-27d41a494660/1669778108485/DestinationPort/parser"
	...
	type ValidationListener struct {
		*parser.BaseDestinationPortListener
	}
	...
	func getGrammarFile() string{
		...
		file := `grammar DestinationPort;

expression :  destinationport EOF ;

destinationport : DESTINATIONPORT ;

DESTINATIONPORT :SPLITKEY  OWNKEY [0-9A-Z]* ;
...
`
	...
	func ValidateInput(input string,runID string,lineHashMap map[int]string) (bool,ErrorInfo){
		...
		lexer:=parser.NewDestinationPortLexer(in)
		if outerMap, ok := ErrorHandleMap.Load(runID); ok {
			if value, canConvert := outerMap.(bool); canConvert {
		...
		parser:=parser.NewDestinationPortParser(stream)
		if outerMap, ok := ErrorHandleMap.Load(runID); ok {
			if value, canConvert := outerMap.(bool); canConvert {
	...
	func HandleCustomError(lineHashMap map[int]string,index int,value string,errorMessage string)ErrorInfo{
		...
		errorInfo.RuleName = "DestinationPort"

	...
	import (
		...
		"strconv"
		"strings"
		parser "FeatureStoreOrg_Default_Department1_Default_Tenant1_validation/Go/d6e7f7a7-351f-4388-b18a-27d41a494660/1675674332598/Disease/parser"
	...
	func getGrammarFile() string{
		...
		file := `grammar Disease;

expression :  disease EOF ;

disease : DISEASE ;

DISEASE :SPLITKEY  OWNKEY ('Chlamydia'|'Gonorrhea'|'Primary and Secondary Syphilis'|'0') ;
...
`
	...
	func ValidateInput(input string,runID string,lineHashMap map[int]string) (bool,ErrorInfo){
		...
		lexer:=parser.NewDiseaseLexer(in)
		if outerMap, ok := ErrorHandleMap.Load(runID); ok {
			if value, canConvert := outerMap.(bool); canConvert {
		...
		parser:=parser.NewDiseaseParser(stream)
		if outerMap, ok := ErrorHandleMap.Load(runID); ok {
			if value, canConvert := outerMap.(bool); canConvert {

	...
	import (
		...
		"strconv"
		"strings"
		parser "FeatureStoreOrg_Default_Department1_Default_Tenant1_validation/Go/d6e7f7a7-351f-4388-b18a-27d41a494660/1672202576415/CropType/parser"
	...
	func getGrammarFile() string{
		...
		file := `grammar CropType;

expression :  croptype EOF ;

croptype : CROPTYPE ;

CROPTYPE :SPLITKEY  OWNKEY ('Coffee'|'Wheat'|'Garden Flowers'|'Groundnuts'|'Maize'|'Paddy'|'Potato'|'Pulse'|'Sugarcane') ;
...
`
	...
	func ValidateInput(input string,runID string,lineHashMap map[int]string) (bool,ErrorInfo){
		...
		lexer:=parser.NewCropTypeLexer(in)
		if outerMap, ok := ErrorHandleMap.Load(runID); ok {
			if value, canConvert := outerMap.(bool); canConvert {
		...
		parser:=parser.NewCropTypeParser(stream)
		if outerMap, ok := ErrorHandleMap.Load(runID); ok {
			if value, canConvert := outerMap.(bool); canConvert {

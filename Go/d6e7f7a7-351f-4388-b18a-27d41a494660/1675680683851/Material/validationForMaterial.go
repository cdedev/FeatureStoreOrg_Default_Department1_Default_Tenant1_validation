	...
	import (
		...
		"strconv"
		"strings"
		parser "FeatureStoreOrg_Default_Department1_Default_Tenant1_validation/Go/d6e7f7a7-351f-4388-b18a-27d41a494660/1675680683851/Material/parser"
	...
	func getGrammarFile() string{
		...
		file := `grammar Material;

expression :  material EOF ;

material : MATERIAL ;

MATERIAL :SPLITKEY  OWNKEY ('Bronze'|'Ceramic'|'Gold/Steel'|'Platinum'|'Red gold'|'Rose gold'|'Silver'|'Steel'|'White gold'|'Yellow gold'|'na') ;
...
`
	...
	func ValidateInput(input string,runID string,lineHashMap map[int]string) (bool,ErrorInfo){
		...
		lexer:=parser.NewMaterialLexer(in)
		if outerMap, ok := ErrorHandleMap.Load(runID); ok {
			if value, canConvert := outerMap.(bool); canConvert {
		...
		parser:=parser.NewMaterialParser(stream)
		if outerMap, ok := ErrorHandleMap.Load(runID); ok {
			if value, canConvert := outerMap.(bool); canConvert {

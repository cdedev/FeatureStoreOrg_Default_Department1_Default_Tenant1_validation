	...
	import (
		...
		"strconv"
		"strings"
		parser "FeatureStoreOrg_Default_Department1_Default_Tenant1_validation/Go/d6e7f7a7-351f-4388-b18a-27d41a494660/1673929021260/Status/parser"
...
expression :  status EOF ;

status : STATUS ;

STATUS :SPLITKEY  OWNKEY ('active'|'deep space'|'engineering'|'failure'|'for orbcomm'|'id reserved'|'inactive'|'launch failed'|'non-amateur'|'non-operational'|'operational'|'project canceled'|'re-entered'|'to be launched'|'unknown'|'unknown'|'weather sat') ;
...
`
	...
	func ValidateInput(input string,runID string,lineHashMap map[int]string) (bool,ErrorInfo){
		...
		lexer:=parser.NewStatusLexer(in)
		if outerMap, ok := ErrorHandleMap.Load(runID); ok {
			if value, canConvert := outerMap.(bool); canConvert {
		...
		parser:=parser.NewStatusParser(stream)
		if outerMap, ok := ErrorHandleMap.Load(runID); ok {
			if value, canConvert := outerMap.(bool); canConvert {

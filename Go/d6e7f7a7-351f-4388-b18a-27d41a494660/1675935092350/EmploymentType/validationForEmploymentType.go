	...
	import (
		...
		"strconv"
		"strings"
		parser "FeatureStoreOrg_Default_Department1_Default_Tenant1_validation/Go/d6e7f7a7-351f-4388-b18a-27d41a494660/1675935092350/EmploymentType/parser"
	...
	type ValidationListener struct {
		*parser.BaseEmploymentTypeListener
	}
	...
	func getGrammarFile() string{
		...
		file := `grammar EmploymentType;

expression :  employmenttype EOF ;

employmenttype : EMPLOYMENTTYPE ;

EMPLOYMENTTYPE :SPLITKEY  OWNKEY ('not in labor force'|'employed'|'unemployed'|'NA'|'irregular_seasonal'|'not_working'|'self_employed'|'other'|'salaried') ;
...
`
	...
	func ValidateInput(input string,runID string,lineHashMap map[int]string) (bool,ErrorInfo){
		...
		lexer:=parser.NewEmploymentTypeLexer(in)
		if outerMap, ok := ErrorHandleMap.Load(runID); ok {
			if value, canConvert := outerMap.(bool); canConvert {
		...
		parser:=parser.NewEmploymentTypeParser(stream)
		if outerMap, ok := ErrorHandleMap.Load(runID); ok {
			if value, canConvert := outerMap.(bool); canConvert {
	...
	func HandleCustomError(lineHashMap map[int]string,index int,value string,errorMessage string)ErrorInfo{
		...
		errorInfo.RuleName = "EmploymentType"

	...
	import (
		...
		"strconv"
		"strings"
		parser "FeatureStoreOrg_Default_Department1_Default_Tenant1_validation/Go/d6e7f7a7-351f-4388-b18a-27d41a494660/1673083556717/Perpetrator/parser"
	...
	type ValidationListener struct {
		*parser.BasePerpetratorListener
	}
	...
	func getGrammarFile() string{
		...
		file := `grammar Perpetrator;

expression :  perpetrator EOF ;

perpetrator : PERPETRATOR ;

PERPETRATOR :SPLITKEY  OWNKEY [A-Za-z -']* ;
...
`
	...
	func ValidateInput(input string,runID string,lineHashMap map[int]string) (bool,ErrorInfo){
		...
		lexer:=parser.NewPerpetratorLexer(in)
		if outerMap, ok := ErrorHandleMap.Load(runID); ok {
			if value, canConvert := outerMap.(bool); canConvert {
		...
		parser:=parser.NewPerpetratorParser(stream)
		if outerMap, ok := ErrorHandleMap.Load(runID); ok {
			if value, canConvert := outerMap.(bool); canConvert {
	...
	func HandleCustomError(lineHashMap map[int]string,index int,value string,errorMessage string)ErrorInfo{
		...
		errorInfo.RuleName = "Perpetrator"

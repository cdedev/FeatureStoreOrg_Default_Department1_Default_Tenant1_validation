	...
	import (
		...
		"strconv"
		"strings"
		parser "FeatureStoreOrg_Default_Department1_Default_Tenant1_validation/Go/d6e7f7a7-351f-4388-b18a-27d41a494660/1674839818224/Joblost/parser"
	...
	func getGrammarFile() string{
		...
		file := `grammar Joblost;

expression :  joblost EOF ;

joblost : JOBLOST ;

JOBLOST :SPLITKEY  OWNKEY ('other'|'slack_work'|'position_abolished'|'seasonal_job_ended') ;
...
`
	...
	func ValidateInput(input string,runID string,lineHashMap map[int]string) (bool,ErrorInfo){
		...
		lexer:=parser.NewJoblostLexer(in)
		if outerMap, ok := ErrorHandleMap.Load(runID); ok {
			if value, canConvert := outerMap.(bool); canConvert {
		...
		parser:=parser.NewJoblostParser(stream)
		if outerMap, ok := ErrorHandleMap.Load(runID); ok {
			if value, canConvert := outerMap.(bool); canConvert {

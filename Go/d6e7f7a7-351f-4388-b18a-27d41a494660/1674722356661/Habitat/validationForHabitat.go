	...
	import (
		...
		"strconv"
		"strings"
		parser "FeatureStoreOrg_Default_Department1_Default_Tenant1_validation/Go/d6e7f7a7-351f-4388-b18a-27d41a494660/1674722356661/Habitat/parser"
	...
	func getGrammarFile() string{
		...
		file := `grammar Habitat;

expression :  habitat EOF ;

habitat : HABITAT ;

HABITAT :SPLITKEY  OWNKEY ('Euryhaline'|'Fresh and brackish water'|'Fresh water'|'Fresh water, brackish water'|'Fresh water, marine'|'Fresh, brackish water'|'Fresh, marine and brackish water'|'Marine'|'Marine and brackish'|'Marine, brackish water'|'Sporadic'|'brackish water') ;
...
`
	...
	func ValidateInput(input string,runID string,lineHashMap map[int]string) (bool,ErrorInfo){
		...
		lexer:=parser.NewHabitatLexer(in)
		if outerMap, ok := ErrorHandleMap.Load(runID); ok {
			if value, canConvert := outerMap.(bool); canConvert {
		...
		parser:=parser.NewHabitatParser(stream)
		if outerMap, ok := ErrorHandleMap.Load(runID); ok {
			if value, canConvert := outerMap.(bool); canConvert {

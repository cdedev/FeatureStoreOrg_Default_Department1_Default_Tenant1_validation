	...
	import (
		...
		"strconv"
		"strings"
		parser "FeatureStoreOrg_Default_Department1_Default_Tenant1_validation/Go/d6e7f7a7-351f-4388-b18a-27d41a494660/1671602041924/DrugsRelatedStop/parser"
	...
	type ValidationListener struct {
		*parser.BaseDrugsRelatedStopListener
	}
	...
	func getGrammarFile() string{
		...
		file := `grammar DrugsRelatedStop;

expression :  drugsrelatedstop EOF ;

drugsrelatedstop : DRUGSRELATEDSTOP ;

DRUGSRELATEDSTOP :SPLITKEY  OWNKEY 'FALSE' ;
...
`
	...
	func ValidateInput(input string,runID string,lineHashMap map[int]string) (bool,ErrorInfo){
		...
		lexer:=parser.NewDrugsRelatedStopLexer(in)
		if outerMap, ok := ErrorHandleMap.Load(runID); ok {
			if value, canConvert := outerMap.(bool); canConvert {
		...
		parser:=parser.NewDrugsRelatedStopParser(stream)
		if outerMap, ok := ErrorHandleMap.Load(runID); ok {
			if value, canConvert := outerMap.(bool); canConvert {
	...
	func HandleCustomError(lineHashMap map[int]string,index int,value string,errorMessage string)ErrorInfo{
		...
		errorInfo.RuleName = "DrugsRelatedStop"

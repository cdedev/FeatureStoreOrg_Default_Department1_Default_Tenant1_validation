	...
	import (
		...
		"strconv"
		"strings"
		parser "FeatureStoreOrg_Default_Department1_Default_Tenant1_validation/Go/d6e7f7a7-351f-4388-b18a-27d41a494660/1669706721715/JobExperience/parser"
	...
	type ValidationListener struct {
		*parser.BaseJobExperienceListener
	}
	...
	func getGrammarFile() string{
		...
		file := `grammar JobExperience;

expression :  jobexperience EOF ;

jobexperience : JOBEXPERIENCE ;

JOBEXPERIENCE :SPLITKEY  OWNKEY [A-Za-z ;]* ;
...
`
	...
	func ValidateInput(input string,runID string,lineHashMap map[int]string) (bool,ErrorInfo){
		...
		lexer:=parser.NewJobExperienceLexer(in)
		if outerMap, ok := ErrorHandleMap.Load(runID); ok {
			if value, canConvert := outerMap.(bool); canConvert {
		...
		parser:=parser.NewJobExperienceParser(stream)
		if outerMap, ok := ErrorHandleMap.Load(runID); ok {
			if value, canConvert := outerMap.(bool); canConvert {
	...
	func HandleCustomError(lineHashMap map[int]string,index int,value string,errorMessage string)ErrorInfo{
		...
		errorInfo.RuleName = "JobExperience"

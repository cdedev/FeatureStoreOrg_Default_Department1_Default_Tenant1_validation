	...
	import (
		...
		"strconv"
		"strings"
		parser "FeatureStoreOrg_Default_Department1_Default_Tenant1_validation/Go/d6e7f7a7-351f-4388-b18a-27d41a494660/1672197300625/Traffic_Density_Score/parser"
	...
	type ValidationListener struct {
		*parser.BaseTraffic_Density_ScoreListener
	}
	...
	func getGrammarFile() string{
		...
		file := `grammar Traffic_Density_Score;

expression :  traffic_density_score EOF ;

traffic_density_score : TRAFFIC_DENSITY_SCORE ;

TRAFFIC_DENSITY_SCORE :SPLITKEY  OWNKEY [0-9.]* ;
...
`
	...
	func ValidateInput(input string,runID string,lineHashMap map[int]string) (bool,ErrorInfo){
		...
		lexer:=parser.NewTraffic_Density_ScoreLexer(in)
		if outerMap, ok := ErrorHandleMap.Load(runID); ok {
			if value, canConvert := outerMap.(bool); canConvert {
		...
		parser:=parser.NewTraffic_Density_ScoreParser(stream)
		if outerMap, ok := ErrorHandleMap.Load(runID); ok {
			if value, canConvert := outerMap.(bool); canConvert {
	...
	func HandleCustomError(lineHashMap map[int]string,index int,value string,errorMessage string)ErrorInfo{
		...
		errorInfo.RuleName = "Traffic_Density_Score"

	...
	import (
		...
		"strconv"
		"strings"
		parser "FeatureStoreOrg_Default_Department1_Default_Tenant1_validation/Go/d6e7f7a7-351f-4388-b18a-27d41a494660/1672078619149/OutdoorParticulateMatter/parser"
	...
	type ValidationListener struct {
		*parser.BaseOutdoorParticulateMatterListener
	}
	...
	func getGrammarFile() string{
		...
		file := `grammar OutdoorParticulateMatter;

expression :  outdoorparticulatematter EOF ;

outdoorparticulatematter : OUTDOORPARTICULATEMATTER ;

OUTDOORPARTICULATEMATTER :SPLITKEY  OWNKEY [0-9.]* ;
...
`
	...
	func ValidateInput(input string,runID string,lineHashMap map[int]string) (bool,ErrorInfo){
		...
		lexer:=parser.NewOutdoorParticulateMatterLexer(in)
		if outerMap, ok := ErrorHandleMap.Load(runID); ok {
			if value, canConvert := outerMap.(bool); canConvert {
		...
		parser:=parser.NewOutdoorParticulateMatterParser(stream)
		if outerMap, ok := ErrorHandleMap.Load(runID); ok {
			if value, canConvert := outerMap.(bool); canConvert {
	...
	func HandleCustomError(lineHashMap map[int]string,index int,value string,errorMessage string)ErrorInfo{
		...
		errorInfo.RuleName = "OutdoorParticulateMatter"

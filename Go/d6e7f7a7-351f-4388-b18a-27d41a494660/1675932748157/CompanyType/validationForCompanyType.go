	...
	import (
		...
		"strconv"
		"strings"
		parser "FeatureStoreOrg_Default_Department1_Default_Tenant1_validation/Go/d6e7f7a7-351f-4388-b18a-27d41a494660/1675932748157/CompanyType/parser"
	...
	type ValidationListener struct {
		*parser.BaseCompanyTypeListener
	}
	...
	func getGrammarFile() string{
		...
		file := `grammar CompanyType;

expression :  companytype EOF ;

companytype : COMPANYTYPE ;

COMPANYTYPE :SPLITKEY  OWNKEY ('Service'|'Product') ;
...
`
	...
	func ValidateInput(input string,runID string,lineHashMap map[int]string) (bool,ErrorInfo){
		...
		lexer:=parser.NewCompanyTypeLexer(in)
		if outerMap, ok := ErrorHandleMap.Load(runID); ok {
			if value, canConvert := outerMap.(bool); canConvert {
		...
		parser:=parser.NewCompanyTypeParser(stream)
		if outerMap, ok := ErrorHandleMap.Load(runID); ok {
			if value, canConvert := outerMap.(bool); canConvert {
	...
	func HandleCustomError(lineHashMap map[int]string,index int,value string,errorMessage string)ErrorInfo{
		...
		errorInfo.RuleName = "CompanyType"

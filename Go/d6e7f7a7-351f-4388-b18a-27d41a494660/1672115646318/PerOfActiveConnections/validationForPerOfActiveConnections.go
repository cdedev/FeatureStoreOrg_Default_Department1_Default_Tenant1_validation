	...
	import (
		...
		"strconv"
		"strings"
		parser "FeatureStoreOrg_Default_Department1_Default_Tenant1_validation/Go/d6e7f7a7-351f-4388-b18a-27d41a494660/1672115646318/PerOfActiveConnections/parser"
	...
	type ValidationListener struct {
		*parser.BasePerOfActiveConnectionsListener
	}
	...
	func getGrammarFile() string{
		...
		file := `grammar PerOfActiveConnections;

expression :  perofactiveconnections EOF ;

perofactiveconnections : PEROFACTIVECONNECTIONS ;

PEROFACTIVECONNECTIONS :SPLITKEY  OWNKEY [0-9]* ;
...
`
	...
	func ValidateInput(input string,runID string,lineHashMap map[int]string) (bool,ErrorInfo){
		...
		lexer:=parser.NewPerOfActiveConnectionsLexer(in)
		if outerMap, ok := ErrorHandleMap.Load(runID); ok {
			if value, canConvert := outerMap.(bool); canConvert {
		...
		parser:=parser.NewPerOfActiveConnectionsParser(stream)
		if outerMap, ok := ErrorHandleMap.Load(runID); ok {
			if value, canConvert := outerMap.(bool); canConvert {
	...
	func HandleCustomError(lineHashMap map[int]string,index int,value string,errorMessage string)ErrorInfo{
		...
		errorInfo.RuleName = "PerOfActiveConnections"

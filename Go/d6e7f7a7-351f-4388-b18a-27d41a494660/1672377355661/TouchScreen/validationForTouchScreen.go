	...
	import (
		...
		"strconv"
		"strings"
		parser "FeatureStoreOrg_Default_Department1_Default_Tenant1_validation/Go/d6e7f7a7-351f-4388-b18a-27d41a494660/1672377355661/TouchScreen/parser"
	...
	type ValidationListener struct {
		*parser.BaseTouchScreenListener
	}
	...
	func getGrammarFile() string{
		...
		file := `grammar TouchScreen;

expression :  touchscreen EOF ;

touchscreen : TOUCHSCREEN ;

TOUCHSCREEN :SPLITKEY  OWNKEY [0-1]* ;
...
`
	...
	func ValidateInput(input string,runID string,lineHashMap map[int]string) (bool,ErrorInfo){
		...
		lexer:=parser.NewTouchScreenLexer(in)
		if outerMap, ok := ErrorHandleMap.Load(runID); ok {
			if value, canConvert := outerMap.(bool); canConvert {
		...
		parser:=parser.NewTouchScreenParser(stream)
		if outerMap, ok := ErrorHandleMap.Load(runID); ok {
			if value, canConvert := outerMap.(bool); canConvert {
	...
	func HandleCustomError(lineHashMap map[int]string,index int,value string,errorMessage string)ErrorInfo{
		...
		errorInfo.RuleName = "TouchScreen"

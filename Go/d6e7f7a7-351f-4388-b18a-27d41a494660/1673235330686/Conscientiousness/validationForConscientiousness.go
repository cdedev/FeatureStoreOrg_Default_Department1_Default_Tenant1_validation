	...
	import (
		...
		"strconv"
		"strings"
		parser "FeatureStoreOrg_Default_Department1_Default_Tenant1_validation/Go/d6e7f7a7-351f-4388-b18a-27d41a494660/1673235330686/Conscientiousness/parser"
	...
	type ValidationListener struct {
		*parser.BaseConscientiousnessListener
	}
	...
	func getGrammarFile() string{
		...
		file := `grammar Conscientiousness;

expression :  conscientiousness EOF ;

conscientiousness : CONSCIENTIOUSNESS ;

CONSCIENTIOUSNESS :SPLITKEY  OWNKEY [1-7.]* ;
...
`
	...
	func ValidateInput(input string,runID string,lineHashMap map[int]string) (bool,ErrorInfo){
		...
		lexer:=parser.NewConscientiousnessLexer(in)
		if outerMap, ok := ErrorHandleMap.Load(runID); ok {
			if value, canConvert := outerMap.(bool); canConvert {
		...
		parser:=parser.NewConscientiousnessParser(stream)
		if outerMap, ok := ErrorHandleMap.Load(runID); ok {
			if value, canConvert := outerMap.(bool); canConvert {
	...
	func HandleCustomError(lineHashMap map[int]string,index int,value string,errorMessage string)ErrorInfo{
		...
		errorInfo.RuleName = "Conscientiousness"

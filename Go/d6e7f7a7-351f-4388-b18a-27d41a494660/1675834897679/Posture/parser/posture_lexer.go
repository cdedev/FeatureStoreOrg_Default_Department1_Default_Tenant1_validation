...
var posturelexerLexerStaticData struct {
	once                   sync.Once
	serializedATN          []int32
	channelNames           []string
	modeNames              []string
...
func posturelexerLexerInit() {
	staticData := &posturelexerLexerStaticData
	...
	staticData.serializedATN = []int32{
		...
		0, 4, 1, 1, 3, 2, 5, 3, 7, 4, 1, 0, 2, 2, 0, 32, 32, 65, 68, 3, 0, 9, 10,
...
// PostureLexerInit initializes any static state used to implement PostureLexer. By default the
// static state used to implement the lexer is lazily initialized during the first call to
// NewPostureLexer(). You can call this function if you wish to initialize the static state ahead
// of time.
func PostureLexerInit() {
	staticData := &posturelexerLexerStaticData
	staticData.once.Do(posturelexerLexerInit)
}

// NewPostureLexer produces a new lexer instance for the optional input antlr.CharStream.
func NewPostureLexer(input antlr.CharStream) *PostureLexer {
	PostureLexerInit()
	l := new(PostureLexer)
...
// PostureLexer tokens.
const (
	PostureLexerPOSTURE  = 1
	PostureLexerOWNKEY   = 2
	PostureLexerSPLITKEY = 3
	PostureLexerWS       = 4
)

	...
	import (
		...
		"strconv"
		"strings"
		parser "FeatureStoreOrg_Default_Department1_Default_Tenant1_validation/Go/d6e7f7a7-351f-4388-b18a-27d41a494660/1676258025950/Clarity/parser"
	...
	func getGrammarFile() string{
		...
		file := `grammar Clarity;

expression :  clarity EOF ;

clarity : CLARITY ;

CLARITY :SPLITKEY  OWNKEY ('I1'|'IF'|'SI1'|'SI2'|'VS1'|'VS2'|'VVS1'|'VVS2'|'FL') ;
...
`
	...
	func ValidateInput(input string,runID string,lineHashMap map[int]string) (bool,ErrorInfo){
		...
		lexer:=parser.NewClarityLexer(in)
		if outerMap, ok := ErrorHandleMap.Load(runID); ok {
			if value, canConvert := outerMap.(bool); canConvert {
		...
		parser:=parser.NewClarityParser(stream)
		if outerMap, ok := ErrorHandleMap.Load(runID); ok {
			if value, canConvert := outerMap.(bool); canConvert {

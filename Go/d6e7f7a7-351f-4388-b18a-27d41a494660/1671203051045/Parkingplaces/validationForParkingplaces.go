	...
	import (
		...
		"strconv"
		"strings"
		parser "FeatureStoreOrg_Default_Department1_Default_Tenant1_validation/Go/d6e7f7a7-351f-4388-b18a-27d41a494660/1671203051045/Parkingplaces/parser"
	...
	type ValidationListener struct {
		*parser.BaseParkingplacesListener
	}
	...
	func getGrammarFile() string{
		...
		file := `grammar Parkingplaces;

expression :  parkingplaces EOF ;

parkingplaces : PARKINGPLACES ;

PARKINGPLACES :SPLITKEY  OWNKEY ('TRUE'|'FALSE') ;
...
`
	...
	func ValidateInput(input string,runID string,lineHashMap map[int]string) (bool,ErrorInfo){
		...
		lexer:=parser.NewParkingplacesLexer(in)
		if outerMap, ok := ErrorHandleMap.Load(runID); ok {
			if value, canConvert := outerMap.(bool); canConvert {
		...
		parser:=parser.NewParkingplacesParser(stream)
		if outerMap, ok := ErrorHandleMap.Load(runID); ok {
			if value, canConvert := outerMap.(bool); canConvert {
	...
	func HandleCustomError(lineHashMap map[int]string,index int,value string,errorMessage string)ErrorInfo{
		...
		errorInfo.RuleName = "Parkingplaces"

	...
	import (
		...
		"strconv"
		"strings"
		parser "FeatureStoreOrg_Default_Department1_Default_Tenant1_validation/Go/d6e7f7a7-351f-4388-b18a-27d41a494660/1674884881303/AvgNoiseLvl/parser"
	...
	type ValidationListener struct {
		*parser.BaseAvgNoiseLvlListener
	}
	...
	func getGrammarFile() string{
		...
		file := `grammar AvgNoiseLvl;

expression :  avgnoiselvl EOF ;

avgnoiselvl : AVGNOISELVL ;

AVGNOISELVL :SPLITKEY  OWNKEY [0-9.]* ;
...
`
	...
	func ValidateInput(input string,runID string,lineHashMap map[int]string) (bool,ErrorInfo){
		...
		lexer:=parser.NewAvgNoiseLvlLexer(in)
		if outerMap, ok := ErrorHandleMap.Load(runID); ok {
			if value, canConvert := outerMap.(bool); canConvert {
		...
		parser:=parser.NewAvgNoiseLvlParser(stream)
		if outerMap, ok := ErrorHandleMap.Load(runID); ok {
			if value, canConvert := outerMap.(bool); canConvert {
	...
	func HandleCustomError(lineHashMap map[int]string,index int,value string,errorMessage string)ErrorInfo{
		...
		errorInfo.RuleName = "AvgNoiseLvl"

	...
	import (
		...
		"strconv"
		"strings"
		parser "FeatureStoreOrg_Default_Department1_Default_Tenant1_validation/Go/d6e7f7a7-351f-4388-b18a-27d41a494660/1675223939602/Ethnicity/parser"
	...
	func getGrammarFile() string{
		...
		file := `grammar Ethnicity;

expression :  ethnicity EOF ;

ethnicity : ETHNICITY ;

ETHNICITY :SPLITKEY  OWNKEY ('Asian'|'Black'|'Latin'|'Middle Eastern'|'Other'|'Prefer not to say'|'White'|'All races') ;
...
`
	...
	func ValidateInput(input string,runID string,lineHashMap map[int]string) (bool,ErrorInfo){
		...
		lexer:=parser.NewEthnicityLexer(in)
		if outerMap, ok := ErrorHandleMap.Load(runID); ok {
			if value, canConvert := outerMap.(bool); canConvert {
		...
		parser:=parser.NewEthnicityParser(stream)
		if outerMap, ok := ErrorHandleMap.Load(runID); ok {
			if value, canConvert := outerMap.(bool); canConvert {

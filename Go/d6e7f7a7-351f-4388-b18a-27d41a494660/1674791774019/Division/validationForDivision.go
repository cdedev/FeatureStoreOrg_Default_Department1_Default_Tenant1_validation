	...
	import (
		...
		"strconv"
		"strings"
		parser "FeatureStoreOrg_Default_Department1_Default_Tenant1_validation/Go/d6e7f7a7-351f-4388-b18a-27d41a494660/1674791774019/Division/parser"
	...
	func getGrammarFile() string{
		...
		file := `grammar Division;

expression :  division EOF ;

division : DIVISION ;

DIVISION :SPLITKEY  OWNKEY ('East North Central'|'East South Central'|'Middle Atlantic'|'Mountain'|'New England'|'Pacific'|'South Atlantic'|'West North Central'|'West South Central') ;
...
`
	...
	func ValidateInput(input string,runID string,lineHashMap map[int]string) (bool,ErrorInfo){
		...
		lexer:=parser.NewDivisionLexer(in)
		if outerMap, ok := ErrorHandleMap.Load(runID); ok {
			if value, canConvert := outerMap.(bool); canConvert {
		...
		parser:=parser.NewDivisionParser(stream)
		if outerMap, ok := ErrorHandleMap.Load(runID); ok {
			if value, canConvert := outerMap.(bool); canConvert {

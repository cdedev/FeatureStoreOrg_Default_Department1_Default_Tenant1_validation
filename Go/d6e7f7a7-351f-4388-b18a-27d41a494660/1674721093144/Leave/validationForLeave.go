	...
	import (
		...
		"strconv"
		"strings"
		parser "FeatureStoreOrg_Default_Department1_Default_Tenant1_validation/Go/d6e7f7a7-351f-4388-b18a-27d41a494660/1674721093144/Leave/parser"
...
expression :  leave EOF ;

leave : LEAVE ;

LEAVE :SPLITKEY  OWNKEY ('Somewhat difficult'|'Somewhat easy'|'Very difficult'|'Very easy'|'Don'[']'t know') ;
...
`
	...
	func ValidateInput(input string,runID string,lineHashMap map[int]string) (bool,ErrorInfo){
		...
		parser:=parser.NewLeaveParser(stream)
		if outerMap, ok := ErrorHandleMap.Load(runID); ok {
			if value, canConvert := outerMap.(bool); canConvert {

...
var violencelexerLexerStaticData struct {
	once                   sync.Once
	serializedATN          []int32
	channelNames           []string
	modeNames              []string
...
func violencelexerLexerInit() {
	staticData := &violencelexerLexerStaticData
...
// ViolenceLexerInit initializes any static state used to implement ViolenceLexer. By default the
// static state used to implement the lexer is lazily initialized during the first call to
// NewViolenceLexer(). You can call this function if you wish to initialize the static state ahead
// of time.
func ViolenceLexerInit() {
	staticData := &violencelexerLexerStaticData
	staticData.once.Do(violencelexerLexerInit)
}

// NewViolenceLexer produces a new lexer instance for the optional input antlr.CharStream.
func NewViolenceLexer(input antlr.CharStream) *ViolenceLexer {
	ViolenceLexerInit()
	l := new(ViolenceLexer)
	...
	l.GrammarFileName = "Violence.g4"
	// TODO: l.EOF = antlr.TokenEOF

	return l
}

// ViolenceLexer tokens.
const (
	ViolenceLexerVIOLENCE = 1
	ViolenceLexerOWNKEY   = 2
	ViolenceLexerSPLITKEY = 3
	ViolenceLexerWS       = 4
)

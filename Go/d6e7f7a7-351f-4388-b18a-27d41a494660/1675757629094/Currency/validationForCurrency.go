	...
	import (
		...
		"strconv"
		"strings"
		parser "FeatureStoreOrg_Default_Department1_Default_Tenant1_validation/Go/d6e7f7a7-351f-4388-b18a-27d41a494660/1675757629094/Currency/parser"
	...
	func getGrammarFile() string{
		...
		file := `grammar Currency;

expression :  currency EOF ;

currency : CURRENCY ;

CURRENCY :SPLITKEY  OWNKEY ('EUR'|'USD'|'AED'|'ARS'|'AUD'|'BHD'|'BRL'|'CAD'|'CHF'|'CLP'|'CNY'|'COP'|'CRC'|'CZK'|'DKK'|'EGP'|'EUR'|'GBP'|'GTQ'|'HKD'|'HNL'|'HRK'|'HUF'|'IDR'|'ILS'|'INR'|'JOD'|'JPY'|'KRW'|'KWD'|'LBP'|'LKR'|'MDL'|'MXN'|'MYR'|'NIO'|'NZD'|'OMR'|'PEN'|'PHP'|'PKR'|'PLN'|'QAR'|'RON'|'RUB'|'SAR'|'SEK'|'SGD'|'THB'|'TRY'|'TWD'|'UAH'|'UYU'|'VEF'|'VND'|'ZAR') ;
...
`
	...
	func ValidateInput(input string,runID string,lineHashMap map[int]string) (bool,ErrorInfo){
		...
		lexer:=parser.NewCurrencyLexer(in)
		if outerMap, ok := ErrorHandleMap.Load(runID); ok {
			if value, canConvert := outerMap.(bool); canConvert {
		...
		parser:=parser.NewCurrencyParser(stream)
		if outerMap, ok := ErrorHandleMap.Load(runID); ok {
			if value, canConvert := outerMap.(bool); canConvert {

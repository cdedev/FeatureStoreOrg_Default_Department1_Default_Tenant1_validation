	...
	import (
		...
		"strconv"
		"strings"
		parser "FeatureStoreOrg_Default_Department1_Default_Tenant1_validation/Go/d6e7f7a7-351f-4388-b18a-27d41a494660/1673235280067/Eonal_stability/parser"
	...
	type ValidationListener struct {
		*parser.BaseEonal_stabilityListener
	}
	...
	func getGrammarFile() string{
		...
		file := `grammar Eonal_stability;

expression :  eonal_stability EOF ;

eonal_stability : EONAL_STABILITY ;

EONAL_STABILITY :SPLITKEY  OWNKEY [1-7.]* ;
...
`
	...
	func ValidateInput(input string,runID string,lineHashMap map[int]string) (bool,ErrorInfo){
		...
		lexer:=parser.NewEonal_stabilityLexer(in)
		if outerMap, ok := ErrorHandleMap.Load(runID); ok {
			if value, canConvert := outerMap.(bool); canConvert {
		...
		parser:=parser.NewEonal_stabilityParser(stream)
		if outerMap, ok := ErrorHandleMap.Load(runID); ok {
			if value, canConvert := outerMap.(bool); canConvert {
	...
	func HandleCustomError(lineHashMap map[int]string,index int,value string,errorMessage string)ErrorInfo{
		...
		errorInfo.RuleName = "Eonal_stability"

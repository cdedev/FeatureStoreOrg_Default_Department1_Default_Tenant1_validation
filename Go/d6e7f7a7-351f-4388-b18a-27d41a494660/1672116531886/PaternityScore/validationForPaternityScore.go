	...
	import (
		...
		"strconv"
		"strings"
		parser "FeatureStoreOrg_Default_Department1_Default_Tenant1_validation/Go/d6e7f7a7-351f-4388-b18a-27d41a494660/1672116531886/PaternityScore/parser"
	...
	type ValidationListener struct {
		*parser.BasePaternityScoreListener
	}
	...
	func getGrammarFile() string{
		...
		file := `grammar PaternityScore;

expression :  paternityscore EOF ;

paternityscore : PATERNITYSCORE ;

PATERNITYSCORE :SPLITKEY  OWNKEY ('Yes'|'No') ;
...
`
	...
	func ValidateInput(input string,runID string,lineHashMap map[int]string) (bool,ErrorInfo){
		...
		lexer:=parser.NewPaternityScoreLexer(in)
		if outerMap, ok := ErrorHandleMap.Load(runID); ok {
			if value, canConvert := outerMap.(bool); canConvert {
		...
		parser:=parser.NewPaternityScoreParser(stream)
		if outerMap, ok := ErrorHandleMap.Load(runID); ok {
			if value, canConvert := outerMap.(bool); canConvert {
	...
	func HandleCustomError(lineHashMap map[int]string,index int,value string,errorMessage string)ErrorInfo{
		...
		errorInfo.RuleName = "PaternityScore"

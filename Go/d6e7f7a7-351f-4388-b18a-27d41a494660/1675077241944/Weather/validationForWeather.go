	...
	import (
		...
		"strconv"
		"strings"
		parser "FeatureStoreOrg_Default_Department1_Default_Tenant1_validation/Go/d6e7f7a7-351f-4388-b18a-27d41a494660/1675077241944/Weather/parser"
	...
	func getGrammarFile() string{
		...
		file := `grammar Weather;

expression :  weather EOF ;

weather : WEATHER ;

WEATHER :SPLITKEY  OWNKEY [0-9.]* ('Clear'|'Cloudy'|'Foggy'|'Rainy'|'Showery'|'Sunny'|'Value not available') ;
...
`
	...
	func ValidateInput(input string,runID string,lineHashMap map[int]string) (bool,ErrorInfo){
		...
		lexer:=parser.NewWeatherLexer(in)
		if outerMap, ok := ErrorHandleMap.Load(runID); ok {
			if value, canConvert := outerMap.(bool); canConvert {
		...
		parser:=parser.NewWeatherParser(stream)
		if outerMap, ok := ErrorHandleMap.Load(runID); ok {
			if value, canConvert := outerMap.(bool); canConvert {

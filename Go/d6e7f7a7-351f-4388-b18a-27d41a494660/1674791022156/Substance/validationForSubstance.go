	...
	import (
		...
		"strconv"
		"strings"
		parser "FeatureStoreOrg_Default_Department1_Default_Tenant1_validation/Go/d6e7f7a7-351f-4388-b18a-27d41a494660/1674791022156/Substance/parser"
	...
	func getGrammarFile() string{
		...
		file := `grammar Substance;

expression :  substance EOF ;

substance : SUBSTANCE ;

SUBSTANCE :SPLITKEY  OWNKEY ('Acetazolamide'|'Anastrozole'|'Benzoylecgonine'|'Betamethasone'|'Betaxolol'|'Bisoprolol'|'Bromazepam'|'Brotizolam'|'Budesonide'|'Caffeine'|'Codeine'|'Dihydrobupropion'|'Ephedrine'|'Heptaminol'|'Hydrochlorothiazide'|'Hydrocodone'|'Lorazepam'|'Methadone'|'Methoxytamoxifen'|'Methylprednisolone'|'Metoprolol'|'Metoprolol acid'|'Morphine'|'N-ethylnicotinamide'|'Oxazepam'|'Prednisolone'|'Prednisone'|'Pseudoephedrine'|'THC'|'Tramadol'|'Zaleplon'|'Zolpidem'|'Zopiclone') ;
...
`
	...
	func ValidateInput(input string,runID string,lineHashMap map[int]string) (bool,ErrorInfo){
		...
		lexer:=parser.NewSubstanceLexer(in)
		if outerMap, ok := ErrorHandleMap.Load(runID); ok {
			if value, canConvert := outerMap.(bool); canConvert {
		...
		parser:=parser.NewSubstanceParser(stream)
		if outerMap, ok := ErrorHandleMap.Load(runID); ok {
			if value, canConvert := outerMap.(bool); canConvert {

	...
	import (
		...
		"strconv"
		"strings"
		parser "FeatureStoreOrg_Default_Department1_Default_Tenant1_validation/Go/d6e7f7a7-351f-4388-b18a-27d41a494660/1672284716001/AlcoholViolence/parser"
	...
	type ValidationListener struct {
		*parser.BaseAlcoholViolenceListener
	}
	...
	func getGrammarFile() string{
		...
		file := `grammar AlcoholViolence;

expression :  alcoholviolence EOF ;

alcoholviolence : ALCOHOLVIOLENCE ;

ALCOHOLVIOLENCE :SPLITKEY  OWNKEY [0-1]* ;
...
`
	...
	func ValidateInput(input string,runID string,lineHashMap map[int]string) (bool,ErrorInfo){
		...
		lexer:=parser.NewAlcoholViolenceLexer(in)
		if outerMap, ok := ErrorHandleMap.Load(runID); ok {
			if value, canConvert := outerMap.(bool); canConvert {
		...
		parser:=parser.NewAlcoholViolenceParser(stream)
		if outerMap, ok := ErrorHandleMap.Load(runID); ok {
			if value, canConvert := outerMap.(bool); canConvert {
	...
	func HandleCustomError(lineHashMap map[int]string,index int,value string,errorMessage string)ErrorInfo{
		...
		errorInfo.RuleName = "AlcoholViolence"

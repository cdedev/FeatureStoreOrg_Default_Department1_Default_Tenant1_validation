	...
	import (
		...
		"strconv"
		"strings"
		parser "FeatureStoreOrg_Default_Department1_Default_Tenant1_validation/Go/d6e7f7a7-351f-4388-b18a-27d41a494660/1673665830093/Consumption/parser"
	...
	type ValidationListener struct {
		*parser.BaseConsumptionListener
	}
	...
	func getGrammarFile() string{
		...
		file := `grammar Consumption;

expression :  consumption EOF ;

consumption : CONSUMPTION ;

CONSUMPTION :SPLITKEY  OWNKEY [0-9C-Zd-z .]* ;
...
`
	...
	func ValidateInput(input string,runID string,lineHashMap map[int]string) (bool,ErrorInfo){
		...
		lexer:=parser.NewConsumptionLexer(in)
		if outerMap, ok := ErrorHandleMap.Load(runID); ok {
			if value, canConvert := outerMap.(bool); canConvert {
		...
		parser:=parser.NewConsumptionParser(stream)
		if outerMap, ok := ErrorHandleMap.Load(runID); ok {
			if value, canConvert := outerMap.(bool); canConvert {
	...
	func HandleCustomError(lineHashMap map[int]string,index int,value string,errorMessage string)ErrorInfo{
		...
		errorInfo.RuleName = "Consumption"

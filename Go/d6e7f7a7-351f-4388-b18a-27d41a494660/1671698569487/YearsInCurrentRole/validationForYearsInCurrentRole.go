	...
	import (
		...
		"strconv"
		"strings"
		parser "FeatureStoreOrg_Default_Department1_Default_Tenant1_validation/Go/d6e7f7a7-351f-4388-b18a-27d41a494660/1671698569487/YearsInCurrentRole/parser"
	...
	type ValidationListener struct {
		*parser.BaseYearsInCurrentRoleListener
	}
	...
	func getGrammarFile() string{
		...
		file := `grammar YearsInCurrentRole;

expression :  yearsincurrentrole EOF ;

yearsincurrentrole : YEARSINCURRENTROLE ;

YEARSINCURRENTROLE :SPLITKEY  OWNKEY [0-9]* ;
...
`
	...
	func ValidateInput(input string,runID string,lineHashMap map[int]string) (bool,ErrorInfo){
		...
		lexer:=parser.NewYearsInCurrentRoleLexer(in)
		if outerMap, ok := ErrorHandleMap.Load(runID); ok {
			if value, canConvert := outerMap.(bool); canConvert {
		...
		parser:=parser.NewYearsInCurrentRoleParser(stream)
		if outerMap, ok := ErrorHandleMap.Load(runID); ok {
			if value, canConvert := outerMap.(bool); canConvert {
	...
	func HandleCustomError(lineHashMap map[int]string,index int,value string,errorMessage string)ErrorInfo{
		...
		errorInfo.RuleName = "YearsInCurrentRole"

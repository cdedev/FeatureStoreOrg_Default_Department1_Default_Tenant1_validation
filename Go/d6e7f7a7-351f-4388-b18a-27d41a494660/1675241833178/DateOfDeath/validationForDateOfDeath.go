	...
	import (
		...
		"strconv"
		"strings"
		parser "FeatureStoreOrg_Default_Department1_Default_Tenant1_validation/Go/d6e7f7a7-351f-4388-b18a-27d41a494660/1675241833178/DateOfDeath/parser"
	...
	type ValidationListener struct {
		*parser.BaseDateOfDeathListener
	}
	...
	func getGrammarFile() string{
		...
		file := `grammar DateOfDeath;

expression :  dateofdeath EOF ;

dateofdeath : DATEOFDEATH ;

DATEOFDEATH :SPLITKEY  OWNKEY [0-9/]* ;
...
`
	...
	func ValidateInput(input string,runID string,lineHashMap map[int]string) (bool,ErrorInfo){
		...
		lexer:=parser.NewDateOfDeathLexer(in)
		if outerMap, ok := ErrorHandleMap.Load(runID); ok {
			if value, canConvert := outerMap.(bool); canConvert {
		...
		parser:=parser.NewDateOfDeathParser(stream)
		if outerMap, ok := ErrorHandleMap.Load(runID); ok {
			if value, canConvert := outerMap.(bool); canConvert {
	...
	func HandleCustomError(lineHashMap map[int]string,index int,value string,errorMessage string)ErrorInfo{
		...
		errorInfo.RuleName = "DateOfDeath"

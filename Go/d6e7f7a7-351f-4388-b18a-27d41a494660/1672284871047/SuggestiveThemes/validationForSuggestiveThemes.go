	...
	import (
		...
		"strconv"
		"strings"
		parser "FeatureStoreOrg_Default_Department1_Default_Tenant1_validation/Go/d6e7f7a7-351f-4388-b18a-27d41a494660/1672284871047/SuggestiveThemes/parser"
	...
	type ValidationListener struct {
		*parser.BaseSuggestiveThemesListener
	}
	...
	func getGrammarFile() string{
		...
		file := `grammar SuggestiveThemes;

expression :  suggestivethemes EOF ;

suggestivethemes : SUGGESTIVETHEMES ;

SUGGESTIVETHEMES :SPLITKEY  OWNKEY [0-1]* ;
...
`
	...
	func ValidateInput(input string,runID string,lineHashMap map[int]string) (bool,ErrorInfo){
		...
		lexer:=parser.NewSuggestiveThemesLexer(in)
		if outerMap, ok := ErrorHandleMap.Load(runID); ok {
			if value, canConvert := outerMap.(bool); canConvert {
		...
		parser:=parser.NewSuggestiveThemesParser(stream)
		if outerMap, ok := ErrorHandleMap.Load(runID); ok {
			if value, canConvert := outerMap.(bool); canConvert {
	...
	func HandleCustomError(lineHashMap map[int]string,index int,value string,errorMessage string)ErrorInfo{
		...
		errorInfo.RuleName = "SuggestiveThemes"

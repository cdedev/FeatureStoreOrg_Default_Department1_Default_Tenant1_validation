	...
	import (
		...
		"strconv"
		"strings"
		parser "FeatureStoreOrg_Default_Department1_Default_Tenant1_validation/Go/d6e7f7a7-351f-4388-b18a-27d41a494660/1674879374317/Species/parser"
	...
	func getGrammarFile() string{
		...
		file := `grammar Species;

expression :  species EOF ;

species : SPECIES ;

SPECIES :SPLITKEY  OWNKEY ('Adelie Penguin (Pygoscelis adeliae)'|'Chinstrap penguin (Pygoscelis antarctica)'|'Gentoo penguin (Pygoscelis papua)') ;
...
`
	...
	func ValidateInput(input string,runID string,lineHashMap map[int]string) (bool,ErrorInfo){
		...
		lexer:=parser.NewSpeciesLexer(in)
		if outerMap, ok := ErrorHandleMap.Load(runID); ok {
			if value, canConvert := outerMap.(bool); canConvert {
		...
		parser:=parser.NewSpeciesParser(stream)
		if outerMap, ok := ErrorHandleMap.Load(runID); ok {
			if value, canConvert := outerMap.(bool); canConvert {

	...
	import (
		...
		"strconv"
		"strings"
		parser "FeatureStoreOrg_Default_Department1_Default_Tenant1_validation/Go/d6e7f7a7-351f-4388-b18a-27d41a494660/1672112976625/DriverPresent/parser"
	...
	type ValidationListener struct {
		*parser.BaseDriverPresentListener
	}
	...
	func getGrammarFile() string{
		...
		file := `grammar DriverPresent;

expression :  driverpresent EOF ;

driverpresent : DRIVERPRESENT ;

DRIVERPRESENT :SPLITKEY  OWNKEY ('Yes'|'No') ;
...
`
	...
	func ValidateInput(input string,runID string,lineHashMap map[int]string) (bool,ErrorInfo){
		...
		lexer:=parser.NewDriverPresentLexer(in)
		if outerMap, ok := ErrorHandleMap.Load(runID); ok {
			if value, canConvert := outerMap.(bool); canConvert {
		...
		parser:=parser.NewDriverPresentParser(stream)
		if outerMap, ok := ErrorHandleMap.Load(runID); ok {
			if value, canConvert := outerMap.(bool); canConvert {
	...
	func HandleCustomError(lineHashMap map[int]string,index int,value string,errorMessage string)ErrorInfo{
		...
		errorInfo.RuleName = "DriverPresent"

	...
	import (
		...
		"strconv"
		"strings"
		parser "FeatureStoreOrg_Default_Department1_Default_Tenant1_validation/Go/d6e7f7a7-351f-4388-b18a-27d41a494660/1672282520870/EverMarried/parser"
	...
	type ValidationListener struct {
		*parser.BaseEverMarriedListener
	}
	...
	func getGrammarFile() string{
		...
		file := `grammar EverMarried;

expression :  evermarried EOF ;

evermarried : EVERMARRIED ;

EVERMARRIED :SPLITKEY  OWNKEY ('Yes'|'No') ;
...
`
	...
	func ValidateInput(input string,runID string,lineHashMap map[int]string) (bool,ErrorInfo){
		...
		lexer:=parser.NewEverMarriedLexer(in)
		if outerMap, ok := ErrorHandleMap.Load(runID); ok {
			if value, canConvert := outerMap.(bool); canConvert {
		...
		parser:=parser.NewEverMarriedParser(stream)
		if outerMap, ok := ErrorHandleMap.Load(runID); ok {
			if value, canConvert := outerMap.(bool); canConvert {
	...
	func HandleCustomError(lineHashMap map[int]string,index int,value string,errorMessage string)ErrorInfo{
		...
		errorInfo.RuleName = "EverMarried"

	...
	import (
		...
		"strconv"
		"strings"
		parser "FeatureStoreOrg_Default_Department1_Default_Tenant1_validation/Go/d6e7f7a7-351f-4388-b18a-27d41a494660/1675076328026/MinorityVote/parser"
	...
	type ValidationListener struct {
		*parser.BaseMinorityVoteListener
	}
	...
	func getGrammarFile() string{
		...
		file := `grammar MinorityVote;

expression :  minorityvote EOF ;

minorityvote : MINORITYVOTE ;

MINORITYVOTE :SPLITKEY  OWNKEY ('Y'|'N') ('Yes'|'No') ;
...
`
	...
	func ValidateInput(input string,runID string,lineHashMap map[int]string) (bool,ErrorInfo){
		...
		lexer:=parser.NewMinorityVoteLexer(in)
		if outerMap, ok := ErrorHandleMap.Load(runID); ok {
			if value, canConvert := outerMap.(bool); canConvert {
		...
		parser:=parser.NewMinorityVoteParser(stream)
		if outerMap, ok := ErrorHandleMap.Load(runID); ok {
			if value, canConvert := outerMap.(bool); canConvert {
	...
	func HandleCustomError(lineHashMap map[int]string,index int,value string,errorMessage string)ErrorInfo{
		...
		errorInfo.RuleName = "MinorityVote"

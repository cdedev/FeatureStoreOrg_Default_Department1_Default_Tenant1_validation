	...
	import (
		...
		"strconv"
		"strings"
		parser "FeatureStoreOrg_Default_Department1_Default_Tenant1_validation/Go/d6e7f7a7-351f-4388-b18a-27d41a494660/1674793422586/BudgetUnit/parser"
	...
	func getGrammarFile() string{
		...
		file := `grammar BudgetUnit;

expression :  budgetunit EOF ;

budgetunit : BUDGETUNIT ;

BUDGETUNIT :SPLITKEY  OWNKEY ('Annual'|'Hourly'|'Monthly') ;
...
`
	...
	func ValidateInput(input string,runID string,lineHashMap map[int]string) (bool,ErrorInfo){
		...
		lexer:=parser.NewBudgetUnitLexer(in)
		if outerMap, ok := ErrorHandleMap.Load(runID); ok {
			if value, canConvert := outerMap.(bool); canConvert {
		...
		parser:=parser.NewBudgetUnitParser(stream)
		if outerMap, ok := ErrorHandleMap.Load(runID); ok {
			if value, canConvert := outerMap.(bool); canConvert {

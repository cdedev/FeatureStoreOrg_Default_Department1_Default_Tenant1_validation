	...
	import (
		...
		"strconv"
		"strings"
		parser "FeatureStoreOrg_Default_Department1_Default_Tenant1_validation/Go/d6e7f7a7-351f-4388-b18a-27d41a494660/1673235220015/Agreeableness/parser"
	...
	type ValidationListener struct {
		*parser.BaseAgreeablenessListener
	}
	...
	func getGrammarFile() string{
		...
		file := `grammar Agreeableness;

expression :  agreeableness EOF ;

agreeableness : AGREEABLENESS ;

AGREEABLENESS :SPLITKEY  OWNKEY [1-7.]* ;
...
`
	...
	func ValidateInput(input string,runID string,lineHashMap map[int]string) (bool,ErrorInfo){
		...
		lexer:=parser.NewAgreeablenessLexer(in)
		if outerMap, ok := ErrorHandleMap.Load(runID); ok {
			if value, canConvert := outerMap.(bool); canConvert {
		...
		parser:=parser.NewAgreeablenessParser(stream)
		if outerMap, ok := ErrorHandleMap.Load(runID); ok {
			if value, canConvert := outerMap.(bool); canConvert {
	...
	func HandleCustomError(lineHashMap map[int]string,index int,value string,errorMessage string)ErrorInfo{
		...
		errorInfo.RuleName = "Agreeableness"

	...
	import (
		...
		"strconv"
		"strings"
		parser "FeatureStoreOrg_Default_Department1_Default_Tenant1_validation/Go/d6e7f7a7-351f-4388-b18a-27d41a494660/1676518353306/Geography/parser"
	...
	func getGrammarFile() string{
		...
		file := `grammar Geography;

expression :  geography EOF ;

geography : GEOGRAPHY ;

GEOGRAPHY :SPLITKEY  OWNKEY ('Barley'|'Coarse grains'|'Corn'|'Energy feeds'|'Feed grains'|'Oats'|'Sorghum'|'France'|'Germany'|'Spain') ;
...
`
	...
	func ValidateInput(input string,runID string,lineHashMap map[int]string) (bool,ErrorInfo){
		...
		lexer:=parser.NewGeographyLexer(in)
		if outerMap, ok := ErrorHandleMap.Load(runID); ok {
			if value, canConvert := outerMap.(bool); canConvert {
		...
		parser:=parser.NewGeographyParser(stream)
		if outerMap, ok := ErrorHandleMap.Load(runID); ok {
			if value, canConvert := outerMap.(bool); canConvert {

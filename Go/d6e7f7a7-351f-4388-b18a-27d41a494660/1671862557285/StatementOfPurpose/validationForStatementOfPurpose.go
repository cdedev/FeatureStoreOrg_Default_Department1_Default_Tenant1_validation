	...
	import (
		...
		"strconv"
		"strings"
		parser "FeatureStoreOrg_Default_Department1_Default_Tenant1_validation/Go/d6e7f7a7-351f-4388-b18a-27d41a494660/1671862557285/StatementOfPurpose/parser"
	...
	type ValidationListener struct {
		*parser.BaseStatementOfPurposeListener
	}
	...
	func getGrammarFile() string{
		...
		file := `grammar StatementOfPurpose;

expression :  statementofpurpose EOF ;

statementofpurpose : STATEMENTOFPURPOSE ;

STATEMENTOFPURPOSE :SPLITKEY  OWNKEY [A-Za-z ]* ;
...
`
	...
	func ValidateInput(input string,runID string,lineHashMap map[int]string) (bool,ErrorInfo){
		...
		lexer:=parser.NewStatementOfPurposeLexer(in)
		if outerMap, ok := ErrorHandleMap.Load(runID); ok {
			if value, canConvert := outerMap.(bool); canConvert {
		...
		parser:=parser.NewStatementOfPurposeParser(stream)
		if outerMap, ok := ErrorHandleMap.Load(runID); ok {
			if value, canConvert := outerMap.(bool); canConvert {
	...
	func HandleCustomError(lineHashMap map[int]string,index int,value string,errorMessage string)ErrorInfo{
		...
		errorInfo.RuleName = "StatementOfPurpose"

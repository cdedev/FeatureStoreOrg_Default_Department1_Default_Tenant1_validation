	...
	import (
		...
		"strconv"
		"strings"
		parser "FeatureStoreOrg_Default_Department1_Default_Tenant1_validation/Go/d6e7f7a7-351f-4388-b18a-27d41a494660/1673673408986/AgeGroup/parser"
	...
	func getGrammarFile() string{
		...
		file := `grammar AgeGroup;

expression :  agegroup EOF ;

agegroup : AGEGROUP ;

AGEGROUP :SPLITKEY  OWNKEY ('17rounder'|'18-24'|'25-34'|'35-44'|'45-54'|'55-64'|'65orover') ;
...
`
	...
	func ValidateInput(input string,runID string,lineHashMap map[int]string) (bool,ErrorInfo){
		...
		lexer:=parser.NewAgeGroupLexer(in)
		if outerMap, ok := ErrorHandleMap.Load(runID); ok {
			if value, canConvert := outerMap.(bool); canConvert {
		...
		parser:=parser.NewAgeGroupParser(stream)
		if outerMap, ok := ErrorHandleMap.Load(runID); ok {
			if value, canConvert := outerMap.(bool); canConvert {

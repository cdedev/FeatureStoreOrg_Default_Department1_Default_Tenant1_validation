	...
	import (
		...
		"strconv"
		"strings"
		parser "FeatureStoreOrg_Default_Department1_Default_Tenant1_validation/Go/d6e7f7a7-351f-4388-b18a-27d41a494660/1672985492567/Vendornoproduits/parser"
	...
	type ValidationListener struct {
		*parser.BaseVendornoproduitsListener
	}
	...
	func getGrammarFile() string{
		...
		file := `grammar Vendornoproduits;

expression :  vendornoproduits EOF ;

vendornoproduits : VENDORNOPRODUITS ;

VENDORNOPRODUITS :SPLITKEY  OWNKEY ('AI'|'ARDO'|'ASAHI'|'BEGA'|'BORDE'|'BOUNDARY'|'BPI'|'CONAPROLE'|'CSM'|'EUROLIVA'|'FALLOT'|'FDL'|'FLECHARD'|'FN'|'GDN'|'GEROLSTEINER'|'GMDF'|'HEINZ'|'HIGHFORD'|'JM'|'KILCOY PASTORAL'|'LAM SOON VIETNAM'|'LB'|'LW'|'MG'|'NPNZ'|'NULL'|'PFANNER'|'PI'|'PL'|'QX'|'RAFAEL'|'TA'|'TARA'|'TURSAN'|'WS') ;
...
`
	...
	func ValidateInput(input string,runID string,lineHashMap map[int]string) (bool,ErrorInfo){
		...
		lexer:=parser.NewVendornoproduitsLexer(in)
		if outerMap, ok := ErrorHandleMap.Load(runID); ok {
			if value, canConvert := outerMap.(bool); canConvert {
		...
		parser:=parser.NewVendornoproduitsParser(stream)
		if outerMap, ok := ErrorHandleMap.Load(runID); ok {
			if value, canConvert := outerMap.(bool); canConvert {
	...
	func HandleCustomError(lineHashMap map[int]string,index int,value string,errorMessage string)ErrorInfo{
		...
		errorInfo.RuleName = "Vendornoproduits"

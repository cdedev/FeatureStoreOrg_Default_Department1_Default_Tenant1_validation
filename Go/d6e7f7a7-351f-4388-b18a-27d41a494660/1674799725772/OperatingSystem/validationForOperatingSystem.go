	...
	import (
		...
		"strconv"
		"strings"
		parser "FeatureStoreOrg_Default_Department1_Default_Tenant1_validation/Go/d6e7f7a7-351f-4388-b18a-27d41a494660/1674799725772/OperatingSystem/parser"
	...
	type ValidationListener struct {
		*parser.BaseOperatingSystemListener
	}
	...
	func getGrammarFile() string{
		...
		file := `grammar OperatingSystem;

expression :  operatingsystem EOF ;

operatingsystem : OPERATINGSYSTEM ;

OPERATINGSYSTEM :SPLITKEY  OWNKEY ('Android'|'iOS') ;
...
`
	...
	func ValidateInput(input string,runID string,lineHashMap map[int]string) (bool,ErrorInfo){
		...
		lexer:=parser.NewOperatingSystemLexer(in)
		if outerMap, ok := ErrorHandleMap.Load(runID); ok {
			if value, canConvert := outerMap.(bool); canConvert {
		...
		parser:=parser.NewOperatingSystemParser(stream)
		if outerMap, ok := ErrorHandleMap.Load(runID); ok {
			if value, canConvert := outerMap.(bool); canConvert {
	...
	func HandleCustomError(lineHashMap map[int]string,index int,value string,errorMessage string)ErrorInfo{
		...
		errorInfo.RuleName = "OperatingSystem"

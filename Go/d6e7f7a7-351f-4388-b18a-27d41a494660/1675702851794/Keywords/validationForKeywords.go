	...
	import (
		...
		"strconv"
		"strings"
		parser "FeatureStoreOrg_Default_Department1_Default_Tenant1_validation/Go/d6e7f7a7-351f-4388-b18a-27d41a494660/1675702851794/Keywords/parser"
	...
	func getGrammarFile() string{
		...
		file := `grammar Keywords;

expression :  keywords EOF ;

keywords : KEYWORDS ;

KEYWORDS :SPLITKEY  OWNKEY ('720BHA22GR00320'|'Food aid,Multiyear'|'Multiyear'|'Multiyear,COVID-19 assistance'|'Rapid Response'|'Rapid response'|'USAID/BHA'|'na') ;
...
`
	...
	func ValidateInput(input string,runID string,lineHashMap map[int]string) (bool,ErrorInfo){
		...
		lexer:=parser.NewKeywordsLexer(in)
		if outerMap, ok := ErrorHandleMap.Load(runID); ok {
			if value, canConvert := outerMap.(bool); canConvert {
		...
		parser:=parser.NewKeywordsParser(stream)
		if outerMap, ok := ErrorHandleMap.Load(runID); ok {
			if value, canConvert := outerMap.(bool); canConvert {

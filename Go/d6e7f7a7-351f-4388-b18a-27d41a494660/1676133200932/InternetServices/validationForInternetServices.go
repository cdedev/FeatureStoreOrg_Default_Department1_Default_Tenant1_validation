	...
	import (
		...
		"strconv"
		"strings"
		parser "FeatureStoreOrg_Default_Department1_Default_Tenant1_validation/Go/d6e7f7a7-351f-4388-b18a-27d41a494660/1676133200932/InternetServices/parser"
	...
	type ValidationListener struct {
		*parser.BaseInternetServicesListener
	}
	...
	func getGrammarFile() string{
		...
		file := `grammar InternetServices;

expression :  internetservices EOF ;

internetservices : INTERNETSERVICES ;

INTERNETSERVICES :SPLITKEY  OWNKEY ('3G'|'4G'|'None'|'na') ;
...
`
	...
	func ValidateInput(input string,runID string,lineHashMap map[int]string) (bool,ErrorInfo){
		...
		lexer:=parser.NewInternetServicesLexer(in)
		if outerMap, ok := ErrorHandleMap.Load(runID); ok {
			if value, canConvert := outerMap.(bool); canConvert {
		...
		parser:=parser.NewInternetServicesParser(stream)
		if outerMap, ok := ErrorHandleMap.Load(runID); ok {
			if value, canConvert := outerMap.(bool); canConvert {
	...
	func HandleCustomError(lineHashMap map[int]string,index int,value string,errorMessage string)ErrorInfo{
		...
		errorInfo.RuleName = "InternetServices"

	...
	import (
		...
		"strconv"
		"strings"
		parser "FeatureStoreOrg_Default_Department1_Default_Tenant1_validation/Go/d6e7f7a7-351f-4388-b18a-27d41a494660/1669654379168/BusinessTravel/parser"
	...
	type ValidationListener struct {
		*parser.BaseBusinessTravelListener
	}
	...
	func getGrammarFile() string{
		...
		file := `grammar BusinessTravel;

expression :  businesstravel EOF ;

businesstravel : BUSINESSTRAVEL ;

BUSINESSTRAVEL :SPLITKEY  OWNKEY [A-Za-z -_]* ;
...
`
	...
	func ValidateInput(input string,runID string,lineHashMap map[int]string) (bool,ErrorInfo){
		...
		lexer:=parser.NewBusinessTravelLexer(in)
		if outerMap, ok := ErrorHandleMap.Load(runID); ok {
			if value, canConvert := outerMap.(bool); canConvert {
		...
		parser:=parser.NewBusinessTravelParser(stream)
		if outerMap, ok := ErrorHandleMap.Load(runID); ok {
			if value, canConvert := outerMap.(bool); canConvert {
	...
	func HandleCustomError(lineHashMap map[int]string,index int,value string,errorMessage string)ErrorInfo{
		...
		errorInfo.RuleName = "BusinessTravel"

...
type TotalWorkingYearsLexer struct {
	*antlr.BaseLexer
	channelNames []string
	modeNames    []string
	// TODO: EOF string
...
var totalworkingyearslexerLexerStaticData struct {
	once                   sync.Once
	serializedATN          []int32
	channelNames           []string
	modeNames              []string
...
func totalworkingyearslexerLexerInit() {
	staticData := &totalworkingyearslexerLexerStaticData
...
// TotalWorkingYearsLexerInit initializes any static state used to implement TotalWorkingYearsLexer. By default the
// static state used to implement the lexer is lazily initialized during the first call to
// NewTotalWorkingYearsLexer(). You can call this function if you wish to initialize the static state ahead
// of time.
func TotalWorkingYearsLexerInit() {
	staticData := &totalworkingyearslexerLexerStaticData
	staticData.once.Do(totalworkingyearslexerLexerInit)
}

// NewTotalWorkingYearsLexer produces a new lexer instance for the optional input antlr.CharStream.
func NewTotalWorkingYearsLexer(input antlr.CharStream) *TotalWorkingYearsLexer {
	TotalWorkingYearsLexerInit()
	l := new(TotalWorkingYearsLexer)
	...
	l.GrammarFileName = "TotalWorkingYears.g4"
	// TODO: l.EOF = antlr.TokenEOF

	return l
}

// TotalWorkingYearsLexer tokens.
const (
	TotalWorkingYearsLexerTOTALWORKINGYEARS = 1
	TotalWorkingYearsLexerOWNKEY            = 2
	TotalWorkingYearsLexerSPLITKEY          = 3
	TotalWorkingYearsLexerWS                = 4
)

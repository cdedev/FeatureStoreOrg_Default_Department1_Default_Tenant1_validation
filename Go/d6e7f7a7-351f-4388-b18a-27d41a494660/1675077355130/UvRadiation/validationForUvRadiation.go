	...
	import (
		...
		"strconv"
		"strings"
		parser "FeatureStoreOrg_Default_Department1_Default_Tenant1_validation/Go/d6e7f7a7-351f-4388-b18a-27d41a494660/1675077355130/UvRadiation/parser"
	...
	type ValidationListener struct {
		*parser.BaseUvRadiationListener
	}
	...
	func getGrammarFile() string{
		...
		file := `grammar UvRadiation;

expression :  uvradiation EOF ;

uvradiation : UVRADIATION ;

UVRADIATION :SPLITKEY  OWNKEY ('high'|'low'|'moderate'|'null'|'very_high') ;
...
`
	...
	func ValidateInput(input string,runID string,lineHashMap map[int]string) (bool,ErrorInfo){
		...
		lexer:=parser.NewUvRadiationLexer(in)
		if outerMap, ok := ErrorHandleMap.Load(runID); ok {
			if value, canConvert := outerMap.(bool); canConvert {
		...
		parser:=parser.NewUvRadiationParser(stream)
		if outerMap, ok := ErrorHandleMap.Load(runID); ok {
			if value, canConvert := outerMap.(bool); canConvert {
	...
	func HandleCustomError(lineHashMap map[int]string,index int,value string,errorMessage string)ErrorInfo{
		...
		errorInfo.RuleName = "UvRadiation"

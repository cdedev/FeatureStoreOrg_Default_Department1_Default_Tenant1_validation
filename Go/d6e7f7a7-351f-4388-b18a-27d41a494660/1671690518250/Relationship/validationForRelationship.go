	...
	import (
		...
		"strconv"
		"strings"
		parser "FeatureStoreOrg_Default_Department1_Default_Tenant1_validation/Go/d6e7f7a7-351f-4388-b18a-27d41a494660/1671690518250/Relationship/parser"
	...
	type ValidationListener struct {
		*parser.BaseRelationshipListener
	}
	...
	func getGrammarFile() string{
		...
		file := `grammar Relationship;

expression :  relationship EOF ;

relationship : RELATIONSHIP ;

RELATIONSHIP :SPLITKEY  OWNKEY [A-Za-z -]* ;
...
`
	...
	func ValidateInput(input string,runID string,lineHashMap map[int]string) (bool,ErrorInfo){
		...
		lexer:=parser.NewRelationshipLexer(in)
		if outerMap, ok := ErrorHandleMap.Load(runID); ok {
			if value, canConvert := outerMap.(bool); canConvert {
		...
		parser:=parser.NewRelationshipParser(stream)
		if outerMap, ok := ErrorHandleMap.Load(runID); ok {
			if value, canConvert := outerMap.(bool); canConvert {
	...
	func HandleCustomError(lineHashMap map[int]string,index int,value string,errorMessage string)ErrorInfo{
		...
		errorInfo.RuleName = "Relationship"

	...
	import (
		...
		"strconv"
		"strings"
		parser "FeatureStoreOrg_Default_Department1_Default_Tenant1_validation/Go/d6e7f7a7-351f-4388-b18a-27d41a494660/1675788016809/PressureLevel/parser"
	...
	type ValidationListener struct {
		*parser.BasePressureLevelListener
	}
	...
	func getGrammarFile() string{
		...
		file := `grammar PressureLevel;

expression :  pressurelevel EOF ;

pressurelevel : PRESSURELEVEL ;

PRESSURELEVEL :SPLITKEY  OWNKEY ('High'|'Low'|'Medium'|'Very High') ;
...
`
	...
	func ValidateInput(input string,runID string,lineHashMap map[int]string) (bool,ErrorInfo){
		...
		lexer:=parser.NewPressureLevelLexer(in)
		if outerMap, ok := ErrorHandleMap.Load(runID); ok {
			if value, canConvert := outerMap.(bool); canConvert {
		...
		parser:=parser.NewPressureLevelParser(stream)
		if outerMap, ok := ErrorHandleMap.Load(runID); ok {
			if value, canConvert := outerMap.(bool); canConvert {
	...
	func HandleCustomError(lineHashMap map[int]string,index int,value string,errorMessage string)ErrorInfo{
		...
		errorInfo.RuleName = "PressureLevel"

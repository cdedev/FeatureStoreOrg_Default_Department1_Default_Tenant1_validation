	...
	import (
		...
		"strconv"
		"strings"
		parser "FeatureStoreOrg_Default_Department1_Default_Tenant1_validation/Go/d6e7f7a7-351f-4388-b18a-27d41a494660/1671601979388/StopDuration/parser"
	...
	type ValidationListener struct {
		*parser.BaseStopDurationListener
	}
	...
	func getGrammarFile() string{
		...
		file := `grammar StopDuration;

expression :  stopduration EOF ;

stopduration : STOPDURATION ;

STOPDURATION :SPLITKEY  OWNKEY [0-9-]* 'Min' ;
...
`
	...
	func ValidateInput(input string,runID string,lineHashMap map[int]string) (bool,ErrorInfo){
		...
		lexer:=parser.NewStopDurationLexer(in)
		if outerMap, ok := ErrorHandleMap.Load(runID); ok {
			if value, canConvert := outerMap.(bool); canConvert {
		...
		parser:=parser.NewStopDurationParser(stream)
		if outerMap, ok := ErrorHandleMap.Load(runID); ok {
			if value, canConvert := outerMap.(bool); canConvert {
	...
	func HandleCustomError(lineHashMap map[int]string,index int,value string,errorMessage string)ErrorInfo{
		...
		errorInfo.RuleName = "StopDuration"

	...
	import (
		...
		"strconv"
		"strings"
		parser "FeatureStoreOrg_Default_Department1_Default_Tenant1_validation/Go/d6e7f7a7-351f-4388-b18a-27d41a494660/1673235389726/Extraversion/parser"
	...
	type ValidationListener struct {
		*parser.BaseExtraversionListener
	}
	...
	func getGrammarFile() string{
		...
		file := `grammar Extraversion;

expression :  extraversion EOF ;

extraversion : EXTRAVERSION ;

EXTRAVERSION :SPLITKEY  OWNKEY [1-7.]* ;
...
`
	...
	func ValidateInput(input string,runID string,lineHashMap map[int]string) (bool,ErrorInfo){
		...
		lexer:=parser.NewExtraversionLexer(in)
		if outerMap, ok := ErrorHandleMap.Load(runID); ok {
			if value, canConvert := outerMap.(bool); canConvert {
		...
		parser:=parser.NewExtraversionParser(stream)
		if outerMap, ok := ErrorHandleMap.Load(runID); ok {
			if value, canConvert := outerMap.(bool); canConvert {
	...
	func HandleCustomError(lineHashMap map[int]string,index int,value string,errorMessage string)ErrorInfo{
		...
		errorInfo.RuleName = "Extraversion"

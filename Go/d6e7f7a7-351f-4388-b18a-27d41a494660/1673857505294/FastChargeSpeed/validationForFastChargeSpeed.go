	...
	import (
		...
		"strconv"
		"strings"
		parser "FeatureStoreOrg_Default_Department1_Default_Tenant1_validation/Go/d6e7f7a7-351f-4388-b18a-27d41a494660/1673857505294/FastChargeSpeed/parser"
	...
	type ValidationListener struct {
		*parser.BaseFastChargeSpeedListener
	}
	...
	func getGrammarFile() string{
		...
		file := `grammar FastChargeSpeed;

expression :  fastchargespeed EOF ;

fastchargespeed : FASTCHARGESPEED ;

FASTCHARGESPEED :SPLITKEY  OWNKEY [0-9]* 'km/h' ;
...
`
	...
	func ValidateInput(input string,runID string,lineHashMap map[int]string) (bool,ErrorInfo){
		...
		lexer:=parser.NewFastChargeSpeedLexer(in)
		if outerMap, ok := ErrorHandleMap.Load(runID); ok {
			if value, canConvert := outerMap.(bool); canConvert {
		...
		parser:=parser.NewFastChargeSpeedParser(stream)
		if outerMap, ok := ErrorHandleMap.Load(runID); ok {
			if value, canConvert := outerMap.(bool); canConvert {
	...
	func HandleCustomError(lineHashMap map[int]string,index int,value string,errorMessage string)ErrorInfo{
		...
		errorInfo.RuleName = "FastChargeSpeed"

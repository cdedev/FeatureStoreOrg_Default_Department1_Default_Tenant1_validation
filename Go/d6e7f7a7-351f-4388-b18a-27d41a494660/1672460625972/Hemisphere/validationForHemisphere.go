	...
	import (
		...
		"strconv"
		"strings"
		parser "FeatureStoreOrg_Default_Department1_Default_Tenant1_validation/Go/d6e7f7a7-351f-4388-b18a-27d41a494660/1672460625972/Hemisphere/parser"
	...
	func getGrammarFile() string{
		...
		file := `grammar Hemisphere;

expression :  hemisphere EOF ;

hemisphere : HEMISPHERE ;

HEMISPHERE :SPLITKEY  OWNKEY ('north'|'south'|'left'|'right') ;
...
`
	...
	func ValidateInput(input string,runID string,lineHashMap map[int]string) (bool,ErrorInfo){
		...
		lexer:=parser.NewHemisphereLexer(in)
		if outerMap, ok := ErrorHandleMap.Load(runID); ok {
			if value, canConvert := outerMap.(bool); canConvert {
		...
		parser:=parser.NewHemisphereParser(stream)
		if outerMap, ok := ErrorHandleMap.Load(runID); ok {
			if value, canConvert := outerMap.(bool); canConvert {

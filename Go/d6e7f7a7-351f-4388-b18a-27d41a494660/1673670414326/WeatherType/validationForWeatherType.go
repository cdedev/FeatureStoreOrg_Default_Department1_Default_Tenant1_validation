	...
	import (
		...
		"strconv"
		"strings"
		parser "FeatureStoreOrg_Default_Department1_Default_Tenant1_validation/Go/d6e7f7a7-351f-4388-b18a-27d41a494660/1673670414326/WeatherType/parser"
	...
	type ValidationListener struct {
		*parser.BaseWeatherTypeListener
	}
	...
	func getGrammarFile() string{
		...
		file := `grammar WeatherType;

expression :  weathertype EOF ;

weathertype : WEATHERTYPE ;

WEATHERTYPE :SPLITKEY  OWNKEY ('Clear'|'Clouds'|'Drizzle'|'Fog'|'Haze'|'Mist'|'Rain'|'Smoke'|'Snow'|'Thunderstorm') ;
...
`
	...
	func ValidateInput(input string,runID string,lineHashMap map[int]string) (bool,ErrorInfo){
		...
		lexer:=parser.NewWeatherTypeLexer(in)
		if outerMap, ok := ErrorHandleMap.Load(runID); ok {
			if value, canConvert := outerMap.(bool); canConvert {
		...
		parser:=parser.NewWeatherTypeParser(stream)
		if outerMap, ok := ErrorHandleMap.Load(runID); ok {
			if value, canConvert := outerMap.(bool); canConvert {
	...
	func HandleCustomError(lineHashMap map[int]string,index int,value string,errorMessage string)ErrorInfo{
		...
		errorInfo.RuleName = "WeatherType"

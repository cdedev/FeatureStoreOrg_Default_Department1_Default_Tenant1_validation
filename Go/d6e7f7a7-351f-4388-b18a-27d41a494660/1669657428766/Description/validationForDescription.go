	...
	import (
		...
		"strconv"
		"strings"
		parser "FeatureStoreOrg_Default_Department1_Default_Tenant1_validation/Go/d6e7f7a7-351f-4388-b18a-27d41a494660/1669657428766/Description/parser"
	...
	type ValidationListener struct {
		*parser.BaseDescriptionListener
	}
	...
	func getGrammarFile() string{
		...
		file := `grammar Description;

expression :  description EOF ;

description : DESCRIPTION ;

DESCRIPTION :SPLITKEY  OWNKEY [0-9A-Za-z .,*/\\;:@#&-?><!~_'"|{}[\]()^%$]* ;
...
`
	...
	func ValidateInput(input string,runID string,lineHashMap map[int]string) (bool,ErrorInfo){
		...
		lexer:=parser.NewDescriptionLexer(in)
		if outerMap, ok := ErrorHandleMap.Load(runID); ok {
			if value, canConvert := outerMap.(bool); canConvert {
		...
		parser:=parser.NewDescriptionParser(stream)
		if outerMap, ok := ErrorHandleMap.Load(runID); ok {
			if value, canConvert := outerMap.(bool); canConvert {
	...
	func HandleCustomError(lineHashMap map[int]string,index int,value string,errorMessage string)ErrorInfo{
		...
		errorInfo.RuleName = "Description"

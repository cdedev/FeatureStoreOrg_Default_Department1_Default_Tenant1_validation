	...
	import (
		...
		"strconv"
		"strings"
		parser "FeatureStoreOrg_Default_Department1_Default_Tenant1_validation/Go/d6e7f7a7-351f-4388-b18a-27d41a494660/1674798883879/Symbol/parser"
...
expression :  symbol EOF ;

symbol : SYMBOL ;

SYMBOL :SPLITKEY  OWNKEY ('AUBANK'|'AXISBANK'|'BANDHANBNK'|'BANKBARODA'|'BANKINDIA'|'CANBK'|'CENTRALBK'|'CSBBANK'|'CUB'|'DCBBANK'|'EQUITASBNK'|'FEDERALBNK'|'HDFC'|'ICICIBANK'|'IDBI'|'IDFCBANK'|'IDFCBANK'|'IDFCFIRSTB'|'INDIANB'|'INDUSINDBK'|'IOB'|'KARURVYSYA'|'KOTAKBANK'|'MAHABANK'|'PNB'|'PSB'|'RBLBANK'|'SBIN'|'SOUTHBANK'|'SURYODAY'|'UCOBANK'|'UJJIVANSFB'|'UNIONBANK'|'YESBANK') ;
...
`
	...
	func ValidateInput(input string,runID string,lineHashMap map[int]string) (bool,ErrorInfo){
		...
		lexer:=parser.NewSymbolLexer(in)
		if outerMap, ok := ErrorHandleMap.Load(runID); ok {
			if value, canConvert := outerMap.(bool); canConvert {
		...
		parser:=parser.NewSymbolParser(stream)
		if outerMap, ok := ErrorHandleMap.Load(runID); ok {
			if value, canConvert := outerMap.(bool); canConvert {
